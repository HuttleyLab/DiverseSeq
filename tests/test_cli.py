...
def test_defaults(runner, tmp_dir, seq_path):
...
def test_min_size(runner, tmp_dir, seq_path, min_size):
    outpath = tmp_dir / "test_min_size.tsv"
    args = f"-s {seq_path} -o {outpath} -z {min_size}".split()
    r = runner.invoke(dvgt_max, args, catch_exceptions=False)
    assert r.exit_code == 0, r.output
...
def test_max_size(runner, tmp_dir, seq_path, max_size):
    outpath = tmp_dir / "test_max_size.tsv"
    args = f"-s {seq_path} -o {outpath} -z 3 -zp {max_size}".split()
    r = runner.invoke(dvgt_max, args, catch_exceptions=False)
    assert r.exit_code == 0, r.output
...
def test_min_eq_max(runner, tmp_dir, seq_path, size):
    outpath = tmp_dir / "test_min_eq_max.tsv"
    args = f"-s {seq_path} -o {outpath} -z {size} -zp {size}".split()
    r = runner.invoke(dvgt_max, args, catch_exceptions=False)
    assert r.exit_code == 0, r.output
    _checked_output(outpath, eval_rows=lambda x: x == size)


def test_min_gt_max_fail(runner, tmp_dir, seq_path):
    outpath = tmp_dir / "test_min_gt_max_fail.tsv"
    args = f"-s {seq_path} -o {outpath} -zp 2".split()
    r = runner.invoke(dvgt_max, args, catch_exceptions=False)
    assert r.exit_code != 0, r.output


@pytest.mark.parametrize("stat", ("total_jsd", "mean_jsd", "mean_delta_jsd"))
def test_stat(runner, tmp_dir, seq_path, stat):
    outpath = tmp_dir / "test_defaults.tsv"
    args = f"-s {seq_path} -o {outpath} -st {stat}".split()
    r = runner.invoke(dvgt_max, args, catch_exceptions=False)
    assert r.exit_code == 0, r.output
...
def test_k(runner, tmp_dir, seq_path, k):
    outpath = tmp_dir / "test_defaults.tsv"
    args = f"-s {seq_path} -o {outpath} -k {k}".split()
    r = runner.invoke(dvgt_max, args, catch_exceptions=False)
    assert r.exit_code == 0, r.output
